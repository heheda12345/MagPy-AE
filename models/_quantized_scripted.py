...
def quantize(x, num_bits:int=8, min_value=None, max_value=None, num_chunks=None, stochastic=False, inplace=False):
...
class QuantMeasure(nn.Module):
    ...
    @torch.jit.ignore
    def _fwd_quant(self, input, min_value, max_value) -> torch.Tensor:
    ...
    def forward(self, input):
        ...
        return self._fwd_quant(input, min_value, max_value)
...
class QConv2d(nn.Conv2d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size,
        ...
        print("create class with torch.jit.ignore", self)

    @torch.jit.ignore
    def _quant_fwd(self, input: torch.Tensor) -> torch.Tensor:
        qinput = self.quantize_input.forward(input) # add .forward as scriptmodule is not callable
...
class QLinear(nn.Linear):
    ...
    @torch.jit.ignore
    def forward(self, input):
        qinput = self.quantize_input.forward(input)
        qweight = quantize(self.weight, num_bits=self.num_bits_weight,
                           min_value=float(self.weight.min()),
                           max_value=float(self.weight.max()))
        if self.bias is not None:
